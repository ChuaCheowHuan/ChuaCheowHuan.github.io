...
The tf.Variable (var) is also declared under the ps device. The value of var is displayed in the for loop.

At the end of the function, the values stored in q will be displayed.
```
...
The tf.Variable (var) is declared under the worker device. It does not has to be declared under the ps device.
